...
-- Add date fields to tracks_v2
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'guardrails_tracks_v2' AND column_name = 'start_date'
  ) THEN
    ALTER TABLE guardrails_tracks_v2 ADD COLUMN start_date date;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'guardrails_tracks_v2' AND column_name = 'end_date'
  ) THEN
    ALTER TABLE guardrails_tracks_v2 ADD COLUMN end_date date;
  END IF;
END $$;
...
-- Create trigger on tracks_v2 for automatic date updates
DROP TRIGGER IF EXISTS trigger_auto_update_parent_track_dates ON guardrails_tracks_v2;
CREATE TRIGGER trigger_auto_update_parent_track_dates
  AFTER INSERT OR UPDATE OR DELETE ON guardrails_tracks_v2
  FOR EACH ROW
...
BEGIN
  RETURN EXISTS (
    SELECT 1 FROM guardrails_tracks_v2
