/**
 * People Guide - Mobile View
 * 
 * Phase 9: Mobile-first People guide using full screen with swipeable sections.
 */

import { useState } from 'react';
import { X, ChevronLeft, ChevronRight, ArrowLeft } from 'lucide-react';
import { PeopleGuideCard } from './PeopleGuideCard';
import { getAllPeopleGuideSections, type PeopleGuideSection } from './peopleGuideContent';

interface PeopleGuideMobileProps {
  isOpen: boolean;
  onClose: () => void;
  onBack?: () => void;
}

export function PeopleGuideMobile({
  isOpen,
  onClose,
  onBack,
}: PeopleGuideMobileProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const sections = getAllPeopleGuideSections();
      ...
      <div className="fixed inset-0 z-50 flex flex-col bg-white safe-top safe-bottom">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6">
          <div className="max-w-md mx-auto">
            <PeopleGuideCard section={currentSection} variant="mobile" />
          </div>
        </div>
