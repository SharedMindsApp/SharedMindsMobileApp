/**
 * Teams Guide - Mobile View
 * 
 * Phase 9: Mobile-first Teams guide using full screen with swipeable sections.
 */

import { useState } from 'react';
import { X, ChevronLeft, ChevronRight, ArrowLeft } from 'lucide-react';
import { TeamsGuideCard } from './TeamsGuideCard';
import { getAllTeamsGuideSections, type TeamsGuideSection } from './teamsGuideContent';

interface TeamsGuideMobileProps {
  isOpen: boolean;
  onClose: () => void;
  onBack?: () => void;
}

export function TeamsGuideMobile({
  isOpen,
  onClose,
  onBack,
}: TeamsGuideMobileProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const sections = getAllTeamsGuideSections();
      ...
      <div className="fixed inset-0 z-50 flex flex-col bg-white safe-top safe-bottom">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6">
          <div className="max-w-md mx-auto">
            <TeamsGuideCard section={currentSection} variant="mobile" />
          </div>
        </div>
