/**
 * Trackers Guide - Mobile View
 * 
 * Phase 9: Mobile-first Trackers guide using full screen with swipeable sections.
 */

import { useState } from 'react';
import { X, ChevronLeft, ChevronRight, ArrowLeft } from 'lucide-react';
import { TrackersGuideCard } from './TrackersGuideCard';
import { getAllTrackersGuideSections, type TrackersGuideSection } from './trackersGuideContent';

interface TrackersGuideMobileProps {
  isOpen: boolean;
  onClose: () => void;
  onBack?: () => void;
}

export function TrackersGuideMobile({
  isOpen,
  onClose,
  onBack,
}: TrackersGuideMobileProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const sections = getAllTrackersGuideSections();
      ...
      <div className="fixed inset-0 z-50 flex flex-col bg-white safe-top safe-bottom">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6">
          <div className="max-w-md mx-auto">
            <TrackersGuideCard section={currentSection} variant="mobile" />
          </div>
        </div>
