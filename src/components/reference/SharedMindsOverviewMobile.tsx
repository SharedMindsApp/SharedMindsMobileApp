/**
 * SharedMinds Overview Guide - Mobile View
 * 
 * Phase 9: Mobile-first SharedMinds overview using full screen with swipeable sections.
 */

import { useState } from 'react';
import { X, ChevronLeft, ChevronRight } from 'lucide-react';
import { SharedMindsOverviewCard } from './SharedMindsOverviewCard';
import { getAllSharedMindsOverviewSections, type SharedMindsOverviewSection } from './sharedMindsOverviewContent';

interface SharedMindsOverviewMobileProps {
  isOpen: boolean;
  onClose: () => void;
}

export function SharedMindsOverviewMobile({
  isOpen,
  onClose,
}: SharedMindsOverviewMobileProps) {
      ...
      <div className="fixed inset-0 z-50 flex flex-col bg-white safe-top safe-bottom">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6">
          <div className="max-w-md mx-auto">
            <SharedMindsOverviewCard section={currentSection} variant="mobile" />
          </div>
        </div>
