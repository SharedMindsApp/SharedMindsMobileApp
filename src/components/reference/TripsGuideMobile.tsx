/**
 * Trips Guide - Mobile View
 * 
 * Phase 9: Mobile-first Trips guide using full screen with swipeable sections.
 */

import { useState } from 'react';
import { X, ChevronLeft, ChevronRight, ArrowLeft } from 'lucide-react';
import { TripsGuideCard } from './TripsGuideCard';
import { getAllTripsGuideSections, type TripsGuideSection } from './tripsGuideContent';

interface TripsGuideMobileProps {
  isOpen: boolean;
  onClose: () => void;
  onBack?: () => void;
}

export function TripsGuideMobile({
  isOpen,
  onClose,
  onBack,
}: TripsGuideMobileProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const sections = getAllTripsGuideSections();
      ...
      <div className="fixed inset-0 z-50 flex flex-col bg-white safe-top safe-bottom">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6">
          <div className="max-w-md mx-auto">
            <TripsGuideCard section={currentSection} variant="mobile" />
          </div>
        </div>
