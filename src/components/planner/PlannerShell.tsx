...
export function PlannerShell({ children }: PlannerShellProps) {
  ...
  // Favourites Bar
  const defaultFavourites = ['/planner', '/planner/daily', '/planner/weekly', '/planner/monthly'];
  ...
  return (
    <>
      ...
      <div className={`min-h-screen bg-gradient-to-br ${stylePreset.colors.bookBg} w-full`}>
        ...
        <div className="flex min-h-screen relative z-10">
          ...
          <aside className="hidden xl:block w-16 flex-shrink-0">
            <div className="sticky top-0 h-screen flex flex-col items-center py-2 gap-1 overflow-y-auto">
              {leftTabs.map((tab) => {
          ...
          <main className="flex-1 min-w-0">
            ...
            <header className={`
              ...
              ${spacing === 'compact' ? 'px-3 sm:px-4 py-2' : 'px-4 sm:px-6 py-3'}
              shadow-lg
            `}>
              <div className="w-full flex items-center justify-between">
                {/* Favourites Tabs - Scrollable on mobile */}
                <div className="flex items-center gap-1 sm:gap-2 overflow-x-auto flex-1 min-w-0 scrollbar-hide">
                  {favouriteTabs.map((tab) => (
                    <button
                      ...
                      className={`
                        ${getTabColorClass(tab.color)}
                        ${isActive(tab.path) ? 'opacity-100 shadow-md' : 'opacity-70 hover:opacity-90'}
                        transition-opacity duration-200
                        ${spacing === 'compact' ? 'px-3 py-1' : 'px-4 py-1.5'} rounded-lg
                ...
                <div className="flex items-center gap-2 ml-4 flex-shrink-0">
                  <button
                    ...
                    className="p-2 text-amber-600 hover:text-amber-700 hover:bg-amber-100/50 rounded-lg transition-colors backdrop-blur-sm"
                    title="Go to Planner Index"
                  >
                    <Home className="w-4 h-4 sm:w-5 sm:h-5" />
                  </button>
                  <button
                    onClick={() => setSettingsOpen(true)}
                    className="p-2 text-gray-700 hover:text-gray-900 hover:bg-white/30 rounded-lg transition-colors backdrop-blur-sm"
            ...
            <div className="relative w-full">
              ...
              <div
                className={`
                  ...
                  pb-20 sm:pb-16 md:pb-12 lg:pb-8
                  relative w-full
                `}
                style={{
                  backgroundImage: `
          ...
          {showSidecar && (
            <aside className={`
              hidden lg:block flex-shrink-0 sticky top-0 h-screen overflow-y-auto scrollbar-hide border-l border-gray-200/50 bg-white/60 backdrop-blur-sm transition-all duration-300
            ...
            `}>
              ...
              {windowWidth > 0 && windowWidth < 1920 && (
                <button
                  ...
                  className="absolute top-1/2 -left-6 z-20 w-12 h-12 bg-blue-600 hover:bg-blue-700 border-2 border-white rounded-full shadow-xl flex items-center justify-center transition-all hover:scale-110 active:scale-95 group"
        ...
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-gray-300 shadow-2xl z-50 safe-area-inset-bottom">
          ...
          <div className="flex items-center border-t border-gray-200">
            <button
              ...
              className={`flex-1 py-3 px-2 text-xs font-bold uppercase transition-colors ${
                mobileMenuSide === 'left' 
                  ? 'bg-blue-100 text-blue-700' 
                  : 'text-gray-600 hover:bg-gray-100'
              }`}
            >
              <Menu className="w-4 h-4 mx-auto mb-1" />
              Time
            </button>
            <button
              onClick={() => {
                ...
                {leftTabs.map((tab) => (
                  <button
                    ...
                    className={`
                      ...
                      ${isActive(tab.path) ? 'opacity-100' : 'opacity-80'}
                      px-4 py-4 text-left text-white font-bold uppercase
                      border-b border-white/20
                      transition-opacity
                      ${isActive(tab.path) ? 'pl-6' : 'pl-4'}
                    `}
                  >
                    {tab.label}
                  </button>
        ...
        <button
          ...
          className="fixed bottom-20 sm:bottom-24 lg:bottom-8 left-4 sm:left-6 lg:left-8 w-14 h-14 sm:w-16 sm:h-16 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-2xl flex items-center justify-center z-40 transition-all hover:scale-110 active:scale-95 shadow-blue-500/50 backdrop-blur-sm"
          title="Quick Actions"
        >
