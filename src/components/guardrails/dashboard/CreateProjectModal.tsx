...
export function CreateProjectModal({
  ...
  async function handleDeleteExistingProject() {
    ...
    const confirmed = window.confirm(
      `Are you sure you want to delete "${existingProject.name}"? This action cannot be undone and will remove all associated data (roadmap, tasks, etc.).`
    );
    ...
    try {
      ...
      alert('Project deleted successfully. You can now create a new project in this domain.');
    } catch (error) {
  ...
  async function handleCreate() {
    ...
    if (domainHasProject) {
      alert('This domain already has a master project. Each domain can only have one master project.');
      return;
    }
                      ...
                      <button
                        ...
                        className={`relative group p-6 rounded-xl transition-all duration-300 transform ${
                          hasProject
                            ...
                            : 'bg-white border-2 border-gray-200 hover:border-transparent hover:shadow-xl hover:scale-105'
                        }`}
                      >
