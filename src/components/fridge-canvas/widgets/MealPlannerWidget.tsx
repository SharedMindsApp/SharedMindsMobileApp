...
export function MealPlannerWidget({ householdId, viewMode, onViewModeChange, onFullscreenChange }: MealPlannerWidgetProps) {
                              ...
                              >
                                {plan ? (
                                  <div className="flex flex-col h-full">
                                    ...
                                    {plan.meal && plan.meal.categories.length > 0 && (
          ...
          <div className="flex-1 overflow-hidden">
            <div className="space-y-1.5">
              {DAYS.slice(0, 3).map((day, index) => {
                const dayMeals = MEAL_TYPES.filter(type => getMealPlan(index, type)).length;
                return (
                  <div key={day} className="bg-white/60 rounded-lg p-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold text-gray-700">{day.slice(0, 3)}</span>
                      <span className="text-xs text-orange-600">{dayMeals}/3</span>
                    </div>
                  </div>
                );
              })}
              <p className="text-xs text-gray-500 text-center mt-2">+{DAYS.length - 3} more days</p>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <>
      <div className="w-full h-full bg-gradient-to-br from-orange-50 to-orange-100 border-orange-300 border-2 rounded-2xl p-3 flex flex-col shadow-lg">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-orange-900 text-sm">Weekly Meal Plan</h3>
              <p className="text-xs text-orange-700">
                {getTotalMeals()} meals planned
              </p>
            </div>
          </div>
          <button
            onClick={() => setShowWeekView(true)}
            className="text-xs px-2 py-1 bg-orange-200 hover:bg-orange-300 text-orange-800 rounded-lg font-semibold transition-colors"
          >
            Week
          </button>
        </div>
        ...
        <div className="flex-1 flex flex-col gap-1 min-h-0 overflow-auto">
          {DAYS.map((day, dayIndex) => (
            <div key={day} className="bg-white rounded-lg p-1.5 border border-orange-200 flex items-center gap-1.5 flex-shrink-0">
              <div className="w-12 flex-shrink-0">
                <span className="font-bold text-gray-900 text-xs">{day.slice(0, 3)}</span>
              </div>
                ...
                {MEAL_TYPES.map(mealType => {
                  ...
                  return (
                    <div
                      key={mealType}
                      onClick={() => !plan && handleAddMeal(dayIndex, mealType)}
                      className={`flex-1 relative border rounded p-1 cursor-pointer transition-all ${
                        plan
                          ? 'bg-green-100 border-green-400'
                          : 'border-gray-300 border-dashed hover:bg-gray-100'
                      }`}
                    >
                      ...
                      {plan && (
                        <button
                          ...
                          className="absolute -top-1 -right-1 bg-white rounded-full p-0.5 text-gray-400 hover:text-gray-600 shadow-sm"
                        >
                          <X size={8} />
                        </button>
                      )}
      ...
      {showWeekView && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-6xl w-full max-h-[90vh] overflow-auto shadow-2xl">
            <div className="sticky top-0 bg-gradient-to-br from-orange-500 to-orange-600 p-6 border-b border-orange-700 z-10">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-white/20 backdrop-blur rounded-xl flex items-center justify-center">
                    <UtensilsCrossed size={24} className="text-white" />
                  ...
                  <div>
                    ...
                    <p className="text-orange-100 text-sm">
                      {getTotalMeals()} meals planned this week
                    </p>
                  </div>
                </div>
                <button
            ...
            <div className="p-6">
              <div className="space-y-4">
                {DAYS.map((day, dayIndex) => (
                  <div key={day} className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-xl p-4 border-2 border-orange-200">
                    <h3 className="font-bold text-gray-900 text-lg mb-3">{day}</h3>
                    <div className="grid grid-cols-3 gap-3">
                      {MEAL_TYPES.map(mealType => {
                        const plan = getMealPlan(dayIndex, mealType);
                        ...
                        return (
                          <div
                            key={mealType}
                            onClick={() => !plan && handleAddMeal(dayIndex, mealType)}
                            className={`relative border-2 rounded-lg p-4 min-h-[120px] transition-all cursor-pointer ${
                              plan
                          ...
                          >
                            {plan ? (
                              <div className="flex flex-col h-full">
                                <div className="flex items-start justify-between gap-2 mb-2">
                                  ...
                                  <button
                                    ...
                                    className="text-gray-400 hover:text-gray-600 hover:bg-white rounded-full p-1"
                                  >
                                    <X size={16} />
                                  </button>
                                </div>
                                ...
                                {plan.meal && (
                                  <div className="mt-auto">
                                    <div className="flex flex-wrap gap-1">
                                      {plan.meal.categories.slice(0, 2).map(cat => (
                                        <span key={cat} className="text-xs px-2 py-0.5 bg-green-200 text-green-800 rounded-full">
                                          {cat.replace('_', ' ')}
                                        </span>
                                      ))}
                                    </div>
                              ...
                              <div className="flex flex-col items-center justify-center h-full text-center">
                                <Plus size={24} className="text-gray-400 mb-2" />
                                <span className="text-sm text-gray-500 capitalize">{mealType}</span>
                                <span className="text-xs text-gray-400 mt-2">Click to add</span>
                              </div>
                            )}
                          </div>
                        );
