...
export async function getUserUIMode(authUserId: string): Promise<UIMode> {
  const { data, error } = await supabase
    .from('profiles')
    .select('ui_mode')
    .eq('user_id', authUserId)
  ...
  return (data?.ui_mode as UIMode) || 'fridge';
}

export async function setUserUIMode(authUserId: string, mode: UIMode): Promise<void> {
